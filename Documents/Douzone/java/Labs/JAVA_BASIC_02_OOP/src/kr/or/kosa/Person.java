...
설계도 : field(고유, 상태, 부품(더 나누어 질 수 있다)) + method(기능, 행위) + constructor(생성자)
...
public class Person {
	// 기본값 설정 
	public String name; //default 값 (자동)으로 할당 (null)  // 생성되는 객체마다 다른 값 가질 수 있음 
	...
	// member field, instance variable >> 객체 변수 >> 주소값을 가지고 있는 변수(메모리를 찾아갈 수 있는 변수)
	
	/*
	 1. instance variable 초기화 하지 않아도 된다 >> why >> default 
